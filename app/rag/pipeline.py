...
class RAGPipeline:
    ...
    def query(self, question: str, topic_filter: str = None, level_filter: str = None) -> ChatResponse:
        ...
        try:
            ...
            retrieved_nodes = retriever.retrieve(safe_question)
            logger.info(f"Retrieved {len(retrieved_nodes)} initial nodes.")
            
            # 3. Reranking
            if retrieved_nodes:
                query_bundle = QueryBundle(query_str=safe_question)
            ...
            # 5. Generation
            raw_response = generate_response(self.llm, context_chunks, safe_question)
